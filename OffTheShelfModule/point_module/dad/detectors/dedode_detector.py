...
from OffTheShelfModule.point_module.dad.utils import get_best_device, sample_keypoints, check_not_i16
from OffTheShelfModule.point_module.dad.types import Detector
...
class DeDoDeDetector(Detector):
    ...
    def load_image(self, im_path, device=get_best_device()) -> dict[str, torch.Tensor]:
        ...
        # 获取当前图像的高度和宽度
        B, C, H, W = im_path.shape

        # 如果需要保持宽高比
        if self.keep_aspect_ratio:
            scale = self.resize / max(H, W)
            new_W = int(scale * W // 8 * 8)  # 调整为 8 的倍数
            new_H = int(scale * H // 8 * 8)  # 调整为 8 的倍数
        else:
            new_H, new_W = self.resize, self.resize

        # 使用 interpolate 进行调整大小
        tensor_resized = F.interpolate(im_path, size=(new_H, new_W), mode='bilinear', align_corners=False)

        # 进行标准化（如果提供了 normalizer）
        tensor_resized = self.normalizer(tensor_resized).float().to(device)
...
def load_DaD(
    ...
    if pretrained:
        ...
        # )
        weights = torch.load(
            'OffTheShelfModule/point_module/weight/dad.pth', weights_only=False, map_location=device
        )
