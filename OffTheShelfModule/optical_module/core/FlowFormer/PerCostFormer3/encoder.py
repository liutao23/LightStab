...
try:
    # timm 1.x 路径
    from timm.layers import Mlp, DropPath, to_2tuple, trunc_normal_, get_act_layer, create_act_layer
    # 做一个“activations”兼容壳，保持后续代码不改
    class _Acts:
        @staticmethod
        def get_act_layer(name):
            return get_act_layer(name)
        @staticmethod
        def create_act_layer(name, **kwargs):
            return create_act_layer(name, **kwargs)
    activations = _Acts
except Exception:
    # 旧版 timm 路径（0.6.x）
    from timm.models.layers import Mlp, DropPath, activations, to_2tuple, trunc_normal_
